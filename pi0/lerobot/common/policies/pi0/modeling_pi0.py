...
class PI0FlowMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Support for extended state and action dimensions
        if self.config.use_extended_dim:
            self.ext_state_proj = nn.Linear(self.config.ext_state_dim, self.config.max_state_dim)
            self.ext_action_in_proj = nn.Linear(self.config.ext_action_dim, self.config.max_action_dim)
            self.ext_action_out_proj = nn.Linear(self.config.max_action_dim, self.config.ext_action_dim)
    ...
    def embed_suffix(self, state, noisy_actions, timestep):
        ...
        # Embed state
        if self.config.use_extended_dim:
            state = self.ext_state_proj(state)
        ...
        if self.config.use_extended_dim:
            noisy_actions = self.ext_action_in_proj(noisy_actions)
    ...
    def forward(
        ...
        if self.config.use_extended_dim:
            v_t = self.ext_action_out_proj(v_t)
    ...
    def sample_actions(self, images, img_masks, lang_tokens, lang_masks, state, noise=None) -> Tensor:
        ...
        if noise is None:
            action_dim = self.config.ext_action_dim if self.config.use_extended_dim else self.config.max_action_dim
            actions_shape = (bsize, self.config.n_action_steps, action_dim)
